...
from env.utils import AverageMeter
#from wrappers import VideoRecorder
...
class CarlaAdapter(gym.core.Env):
    ...
    def _compute_reward(self):
        ...
        reward_c = -1*self._collision_penalty * self._impulse_intensity
        # penalize steering
        reward -= self._steer_penalty * abs(self._control.steer)
        reward_s = -1*self._steer_penalty * abs(self._control.steer)
        return reward
    ...
    def _get_distance_traveled(self):
        _ego_v = self._player.get_velocity() # unit: m/s
        _velocity_x =  _ego_v.x
        _velocity_y =  _ego_v.y
        _velocity_z =  _ego_v.z

        _delta_t = self._delta_t

        _dist_x = _velocity_x * _delta_t
        _dist_y = _velocity_y * _delta_t
        _dist_z = _velocity_z * _delta_t
        _dist_traveled = math.sqrt(_dist_x ** 2 + _dist_y ** 2 + _dist_z ** 2)
    ...
    def step(self, action):
        ...
        for _ in range(self._action_repeat):
            ...
            dist_travelled += self._get_distance_traveled()
            done = self._is_done()
        ...
        extra = {
            ...
            "dist_travelled_meters": dist_travelled,
        }
    ...
    def reset(self):
        ...
        #self._player.apply_control(carla.VehicleControl(throttle=1.0, brake=1.0))
        #time.sleep(4.0)
        #while True:
        #    _ego_v = self._player.get_velocity()
        #    if abs(_ego_v.x) > 0.5 or abs(_ego_v.y) > 0.5 or abs(_ego_v.z) > 0.5:
        #        break
        #    self._tick()
        #self._player.apply_control(carla.VehicleControl(throttle=1.0, brake=0.0))
        self._tick()
        if self._k is not None:
            _obs = self._get_obs()
