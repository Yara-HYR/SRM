...
def get_obs_space(
    ...
    shape[0] = k*shape[0] if k else shape[0]
...
def get_stacked_obs(frames, modalities):
    ...
    if "rgb" in modalities:
        ...
        rgb_stacked = []
        for frame in frames:
            #rgb_stacked.append(np.expand_dims(frame["rgb"], 0)) # old, original
            rgb_stacked.append(frame["rgb"]) # new, for hyr
